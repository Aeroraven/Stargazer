...
import { AriaLoadIndicator } from "./aria-load-indicator";
...
export class AriaAssetLoader{
    ...
    private async load(){
        ...
        for(let el of shaderLists){
            AriaLoadIndicator.getInstance().updateLoadingTip("Resources/Shader/"+el.name)
        ...
        for(let el of textureLists){
            AriaLoadIndicator.getInstance().updateLoadingTip("Resources/Texture/"+el.name)
