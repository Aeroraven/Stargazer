...
export class AriaStageSSR extends AriaStage{
    renderEnt:()=>void
    ...
    public async prepare(gl: WebGL2RenderingContext): Promise<any> {
        ...
        const normalFramebufOpt = AriaFramebufferOption.create().setHdr(true).setScaler(1)
        const normalFramebuf = new AriaFramebuffer(gl,normalFramebufOpt)
        assets.addTexture("klee/normal", normalFramebuf.tex)

        const posFramebufOpt = AriaFramebufferOption.create().setHdr(true).setScaler(1)
        const posFramebuf = new AriaFramebuffer(gl,posFramebufOpt)
        assets.addTexture("klee/depth", posFramebuf.tex)

        const ssrFramebufOpt = AriaFramebufferOption.create().setHdr(true).setScaler(0.8)
        const ssrFramebuf = new AriaFramebuffer(gl,ssrFramebufOpt)
        assets.addTexture("klee/ssr", ssrFramebuf.tex)
        ...
        assets.addTexture("klee/org", orgFramebuf.tex)

        const finalFramebufOpt = AriaFramebufferOption.create().setHdr(true).setScaler(1)
        const finalFramebuf = new AriaFramebuffer(gl,finalFramebufOpt)
        assets.addTexture("klee/final", finalFramebuf.tex)

        const gaFramebufOpt = AriaFramebufferOption.create().setHdr(true).setScaler(1)
        const gaFramebuf = new AriaFramebuffer(gl,gaFramebufOpt)
        assets.addTexture("klee/ga", gaFramebuf.tex)

        const gbFramebufOpt = AriaFramebufferOption.create().setHdr(true).setScaler(1)
        const gbFramebuf = new AriaFramebuffer(gl,gbFramebufOpt)
        assets.addTexture("klee/gb", gbFramebuf.tex)
        ...
        const ssrTex = (<AriaComTexture>AriaComTexture.create(gl))
            .setTex(assets.getTexture("klee/ssr"))
        const orgTex = (<AriaComTexture>AriaComTexture.create(gl))
        ...
        const gaTex = (<AriaComTexture>AriaComTexture.create(gl))
            .setTex(assets.getTexture("klee/ga"))
        const gbTex = (<AriaComTexture>AriaComTexture.create(gl))
            .setTex(assets.getTexture("klee/gb"))
        //Floor
        ...
        //Postproc pass
        const ssrPass = (<AriaComPostprocessPass>AriaComPostprocessPass.create(gl))
            .setCamera(camera)
            .setShader(assets.getShader("klee-ssr/reflection"))
            .setTexture(AriaComMeshTextureType.acmtDiffuse,posTex)
            .setTexture(AriaComMeshTextureType.acmtSpecular,normalTex)
            .setTexture(AriaComMeshTextureType.acmtNormal,orgTex)

        const gaussianAPass = (<AriaComPostprocessPass>AriaComPostprocessPass.create(gl))
            .setCamera(camera)
            .setShader(assets.getShader("gaussian-horizontal"))
            .setTexture(AriaComMeshTextureType.acmtDiffuse,ssrTex)

        const gaussianBPass = (<AriaComPostprocessPass>AriaComPostprocessPass.create(gl))
            .setCamera(camera)
            .setShader(assets.getShader("gaussian-vertical"))
            .setTexture(AriaComMeshTextureType.acmtDiffuse,gaTex)

            
        const postPass = (<AriaComPostprocessPass>AriaComPostprocessPass.create(gl))
            .setCamera(camera)
            .setShader(assets.getShader("klee-ssr/postmix"))
            .setTexture(AriaComMeshTextureType.acmtDiffuse,gbTex)
            .setTexture(AriaComMeshTextureType.acmtSpecular,orgTex)
        ...
        const deferPosSetup = ()=>{
            kleeMeshes.forEach((el)=>{
                el.setShader(assets.getShader("klee-ssr/coarsepos"))
            })
            floorMesh.setShader(assets.getShader("klee-ssr/reflpos"))
        }
        ...
        const deferUnload = ()=>{
            kleeMeshes.forEach((el)=>{
                el.setShader(assets.getShader("klee/scene"))
            })
        ...
        const renderFunc = ()=>{
            ...
            gaFramebuf.bind()
            clearScene(gl)
            gaussianAPass.render()
            gaFramebuf.unbind()

            gbFramebuf.bind()
            clearScene(gl)
            gaussianBPass.render()
            gbFramebuf.unbind()
